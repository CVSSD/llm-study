...
# 梯度历史：grad_history[epoch] = [[g3,g2,g1], [g3,g2,g1], ...]（每个采样时刻一行）
# g3/y_hat_3 = 最深隐藏层，g1/y_hat_1 = 最浅隐藏层（反向传播方向：输出→输入）
sample_every = 25   # 每 25 个 batch 采样一次（60000/128≈469 batch/epoch → 约 19 个点）
grad_history = []
...
for epoch in range(num_epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        # 采样必须在 backward 之后、下一个 batch 覆盖 grad 之前
        if i % sample_every == 0:
            g3 = y_hat_3.grad.norm().item()
            g2 = y_hat_2.grad.norm().item()
            g1 = y_hat_1.grad.norm().item()
            epoch_grads.append([g3, g2, g1])
            # Ratios in the backward direction: y_hat_3 -> y_hat_2 -> y_hat_1.
            epoch_gradient_ratios.append([
                g2 / max(g3, 1e-12),
                g1 / max(g2, 1e-12),
            ])
...
ax2.plot(train_acc, label="Train Acc"); ax2.plot(test_acc, label="Test Acc")
...
fig.savefig(save_dir / f'4layer_{num_epoch}_lr{lr}_mlp_3.png', dpi=150, bbox_inches='tight')

# ===== 梯度 vs 层：1x3 子图（epoch 1、15、30）=====
# 每子图内：x=层位置（输出→输入），y=log10(梯度范数)
# 多条线 = 该 epoch 内不同采样时刻（颜色从浅到深 = 训练从早到晚）
layer_x = [3, 2, 1]   # y_hat_3 -> y_hat_2 -> y_hat_1
layer_labels = ['y_hat_3', 'y_hat_2', 'y_hat_1']
target_epochs = [0, 29, 44, 59, 70, 99]   # 第 1、15、30 个 epoch（0-based 索引）

fig2, axes = plt.subplots(2, 3, figsize=(15, 5))
axes = axes.flatten()
...
plt.tight_layout()
fig2.savefig(save_dir / f'4layer_{num_epoch}_lr{lr}_grad_vs_layer.png', dpi=150, bbox_inches='tight')
...
fig3.tight_layout()
fig3.savefig(save_dir / '4layer_near_zero_weights.png', dpi=150,
             bbox_inches='tight')
...
fig4.tight_layout()
fig4.savefig(save_dir / '4layer_persistent_zero_weights.png', dpi=150,
             bbox_inches='tight')

# log10 ratios: negative values mean attenuation toward the input.
fig5, axes = plt.subplots(2, 3, figsize=(15, 8))
axes = axes.flatten()
ratio_x = np.arange(2)
ratio_labels = [r'$g_2/g_3$', r'$g_1/g_2$']
for ax, ep in zip(axes, target_epochs):
    ...
    ax.set_xlabel('Backward transitions')
    ax.set_ylabel(r'$\log_{10}(gradient\ ratio)$')
    ax.grid(True, alpha=0.3)

fig5.tight_layout()
fig5.savefig(save_dir / '4layer_gradient_change_rate.png', dpi=150,
             bbox_inches='tight')
