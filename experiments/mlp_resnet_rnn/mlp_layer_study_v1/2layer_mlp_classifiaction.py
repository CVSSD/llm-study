...
# Each sample follows the forward order:
# [Linear 1 output (pre-ReLU), ReLU 1 output, Linear 2 output (logits)].
sample_every = 25
grad_history = []
...
for epoch in range(num_epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        linear_1 = X @ w1 + b1
        linear_1.retain_grad()
        relu_1 = ReLu(linear_1)
        relu_1.retain_grad()
        y_hat = relu_1 @ w2 + b2
        y_hat.retain_grad()
        ...
        if i % sample_every == 0:
            epoch_grads.append([
                linear_1.grad.norm().item(),
                relu_1.grad.norm().item(),
                y_hat.grad.norm().item(),
            ])
            current_zero_masks = [
...
# Gradient norm at the output of every operation. The plot is in forward order;
# gradients propagate from Linear 2 back toward Linear 1 (right to left).
layer_x = np.arange(3)
layer_labels = ['Linear 1\n(pre-ReLU)', 'ReLU 1\n(post-ReLU)',
                'Linear 2\n(logits)']
...
fig2.tight_layout()
fig2.savefig(save_dir / '2layer_grad_vs_layer.png', dpi=150, bbox_inches='tight')
...
fig3.tight_layout()
fig3.savefig(save_dir / '2layer_near_zero_weights.png', dpi=150,
             bbox_inches='tight')
...
fig4.tight_layout()
fig4.savefig(save_dir / '2layer_persistent_zero_weights.png', dpi=150,
             bbox_inches='tight')
plt.show()
